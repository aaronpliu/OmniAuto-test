...
A TypeScript-based test automation framework supporting **iOS (Detox / Appium XCUITest)**, **Android (Appium UiAutomator2 / Detox)**, **Web (Playwright)**, and **API testing** — all from the same test scripts.
...
| 🎯 **Unified Selectors** | `by.id()` / `by.text()` / `by.label()` / `by.xpath()` work across all platforms |
...
- **Test Steps**: Every action automatically logged (click, typeText, expect*, waitFor*)
