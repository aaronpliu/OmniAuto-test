...
基于 TypeScript 的测试自动化框架，支持 **iOS（Detox / Appium XCUITest）**、**Android（Appium UiAutomator2 / Detox）**、**Web（Playwright）** 和 **API 测试**，同一套测试脚本覆盖所有平台。
...
| 🎯 **统一选择器** | `by.id()` / `by.text()` / `by.label()` / `by.xpath()` 跨平台通用 |
