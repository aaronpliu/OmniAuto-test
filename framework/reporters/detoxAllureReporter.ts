...
function drainTestAttachments(): { name: string; type: string; source: string }[] {
  try {
    ...
    raw
      ...
      .filter(Boolean)
      .forEach((line: string) => {
        const item = JSON.parse(line) as { screenshot?: string };
      ...
      (s: {
        ...
        if (s.screenshot && existsSync(s.screenshot)) {
          try {
            ...
            result.attachments = [{ name: "Step Screenshot", type: "image/png", source: destName }];
          } catch {
            // 复制失败不影响步骤记录
          }
        }
...
class DetoxAllureReporter implements jest.Reporter {
  ...
  onTestResult(_test: jest.Test, testResult: jest.TestResult): void {
    ...
    for (const result of testResult.testResults) {
      ...
      const isSkipped =
        result.status === "skipped" || result.status === "pending" || result.status === "disabled";
      ...
      const allureResult: AllureTestResult = {
        ...
        statusDetails:
          !isSkipped && allSteps.length === 0 && result.failureMessages?.length
  ...
  private collectDetoxArtifacts(): void {
    ...
    if (!existsSync(detoxArtifactsDir)) {
      return;
    }
    ...
    try {
      runDirs = readdirSync(detoxArtifactsDir).filter((name) => {
        try {
          return statSync(join(detoxArtifactsDir, name)).isDirectory();
        } catch {
          return false;
        }
      });
    ...
    for (const runDir of runDirs) {
      ...
      try {
        testDirs = readdirSync(runPath).filter((name) => {
          try {
            return statSync(join(runPath, name)).isDirectory();
          } catch {
            return false;
          }
        });
      ...
      for (const testDir of testDirs) {
        ...
        try {
          files = readdirSync(testPath).filter((name) => {
            try {
              return statSync(join(testPath, name)).isFile();
            } catch {
              return false;
            }
          });
        ...
        if (newAttachments.length > 0) {
          ...
          if (result) {
            ...
            console.log(
              `[AllureReporter] Attached ${newAttachments.length} Detox artifacts to: ${testFullName}`
            );
          }
        }
      }
  ...
  private findResultByFullName(testFullName: string): string | undefined {
    ...
    for (const [uuid, result] of this.results) {
      if (result.fullName === testFullName) {
        return uuid;
      }
    }
    // 模糊匹配兜底
