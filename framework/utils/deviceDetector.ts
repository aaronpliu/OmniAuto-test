...
export class DeviceDetector {
  ...
  async detectDevices(): Promise<DeviceInfo[]> {
    try {
      ...
      devices.forEach(device => {
        logger.info(`  - ${device.udid} (${device.model}) ${device.isEmulator ? '[模拟器]' : '[真机]'} Android ${device.version}`);
      });
   ...
   * 获取设备详细信息
   */
  private async getDeviceDetails(udid: string): Promise<{ model: string; version: string; isEmulator: boolean }> {
    try {
  ...
  async detectAndSelectDevice(): Promise<DeviceInfo> {
    ...
    if (devices.length === 0) {
      throw new Error('未检测到可用的 Android 设备，请确保：\n1. 设备已连接\n2. 设备已授权 USB 调试\n3. adb 已正确安装');
    }
