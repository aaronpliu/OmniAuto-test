...
import { HomePage } from "@tests/mobile/your-app/pages/HomePage";
...
describe("Advanced Cross-Platform Tests", () => {
  ...
  it("should handle complex elements (platform-specific)", async () => {
    if (isDetoxActions(actions)) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-var-requires -- dynamic import for detox matchers
      const detox = require("detox");
      const complexElement = DetoxActions.byAll(
        detox.by.id("welcome-text"),
        detox.by.text("Welcome")
      );
