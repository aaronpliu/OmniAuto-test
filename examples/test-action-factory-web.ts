...
const mockPage = {
  ...
  locator: () => ({
    boundingBox: async () => ({ width: 800, height: 600 }),
...
async function testActionFactory() {
  ...
  try {
    ...
    const actions2 = ActionFactory.createForWeb(mockPage as any, mockBrowser as any);
    ...
    const actions3 = ActionFactory.create({
      platform: 'web',
    ...
    const requiredMethods = [
      'navigateTo', 'click', 'doubleClick', 'tap', 'longPress',
      'typeText', 'clearText', 'getText',
      'waitForElement', 'expectVisible', 'expectNotVisible',
      'expectText', 'expectContainsText', 'expectEnabled', 'expectDisabled',
      'swipe', 'scroll', 'pinch',
      'takeScreenshot', 'reload', 'back', 'close',
      'setOrientation', 'setLocation'
    ];
