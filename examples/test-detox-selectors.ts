...
(global as any).device = {
  launchApp: async () => {},
  takeScreenshot: async () => '/path/to/screenshot.png',
...
async function testSelectorResolution() {
  ...
  try {
    ...
    const combinedMatcher = DetoxActions.byAll(
      by.id('btn'),
      by.text('Submit')
    );
    ...
    const stringSelector: string = 'button';
    const nativeElementSelector = element(by.id('button'));
    const textMatcherSelector = by.text('Click me');
    const labelMatcherSelector = by.label('Action button');
